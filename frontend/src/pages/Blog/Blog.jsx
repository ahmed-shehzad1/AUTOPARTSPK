  ...
  if (posts.length === 0) {
    return (
      <div className="bg-steel min-h-screen flex items-center justify-center">
        <p className="font-body text-slate">No articles published yet.</p>
      </div>
    )
  }
        ...
        <Reveal>
          ...
          >
            ...
            <div className="flex items-center gap-4 font-mono text-xs text-slate/50">
              <span>{new Date(featured.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
